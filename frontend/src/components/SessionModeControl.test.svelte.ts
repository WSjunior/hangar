...
it('não chama o modo desconhecido de Normal e permite escolher explicitamente', async () => {
  const onApply = vi.fn();
  const target = await render({ provider: 'codex', current: null, onApply });
  const trigger = target.querySelector<HTMLButtonElement>('[data-mode-trigger]')!;
  expect(trigger.textContent?.trim()).toBe(m.chat_mode_label());
  trigger.click(); await tick();
  document.querySelector<HTMLButtonElement>('[data-mode="plan"]')!.click();
