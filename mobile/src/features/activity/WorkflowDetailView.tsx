...
interface Props {
  ...
  // Separado do `detail` ausente: falha de rede pede tentar de novo, run inexistente não.
  erro?: string;
  onTentar?: () => void;
}
...
export function WorkflowDetailView({ detail, loading, onAgent, erro, onTentar }: Props) {
  ...
  if (erro) {
    return (
      <Text style={[styles.empty, { color: theme.tokens.status.warning }]} onPress={onTentar} accessibilityRole="button">
        ⚠ {erro}
      </Text>
    );
  }
